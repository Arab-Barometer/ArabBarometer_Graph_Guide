...
As a quick review, in earlier chapters the first step was to create a list of summaries by give the summarize function a vector of variable names. Next we would map that list of summaries to the related plot function. In the mapping function, we would hold the caption constant since we wanted the same caption to appear on every graph.

The `plot_trend_individual()` function creates summaries internally, so we can "skip" the first step. Here we go right to the mapping step. We map a vector of variable names to the `plot_trend_individual()` function. We want to create a graph for the same country over the same time period, just using different variable, so we hold the data frame list, survey dates, country, *and* caption constant.
