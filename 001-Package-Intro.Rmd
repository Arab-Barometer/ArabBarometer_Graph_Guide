...
The function `add_demographics()` preforms all the functions above: `recode_country()`, `gender_demographic()`, `age_demographic()`, `income_demographic()`, `education_demographic()`, and `settlement_demographic()`. You do **not** need to use the above functions if you use `add_demographics()`.
...
```{r add-demographics-ex, include=TRUE,eval=FALSE}
...
df_3 <- df %>%            # Data frame
  dummy_all(c,            # Variable to dummy
            no_NA = FALSE # Include dummy for NA occurances
            )
